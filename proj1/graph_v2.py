...
def to_hundred_thousandths(v):
    """
    >>> -11389761 == to_hundred_thousandths(-113.897610)
    True
    """
    return math.floor(100000 * float(v))

def from_hundred_thousandths(v):
    """
    >>> print("%.6f"%(from_hundred_thousandths(-5971710)))
    -59.717100
    """
...
def parse_vertex(split_fields):
    """
    >>> v1 = parse_vertex(('V', 282, -54.28281, -114.82982))
    >>> v1 == {'id': 282, 'lat': -5428281, 'lon': -11482982}
    True
    >>> # Checking out the case in which there are extra fields
    >>> v2 = parse_vertex(('V', 282, -54.28281, -114.82982, 928271))
    >>> v2 == {'id': 282, 'lat': -5428281, 'lon': -11482982}
    True
    """
    _, vId, lat, lon, *rest = split_fields
    # Using the { format for the sake of prettifying the results
    return {
        'id':   to_id(vId),
        'lat':  to_hundred_thousandths(lat),
        'lon':  to_hundred_thousandths(lon)
    }

def parse_edge(split_fields):
    """
    >>> e1 = parse_edge(('E', 1, 2, 'Angola'))
    >>> e1 == {'start': 1, 'end': 2, 'name': 'Angola'}
    True
    >>> # Checking out the case in which there are extra fields
    >>> e2 = parse_edge(('E', 1, 2, 'Angola', 'Zurich'))
    >>> e2 == {'start': 1, 'end': 2, 'name': 'Angola'}
    True
    """

    _, start, end, name, *rest = split_fields
    return dict(start=start, name=name, end=end)

def noop(*args, **kwargs):
    return dict(noop=True)
...
def read_undirected_city_graph(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            parser = noop
            splits = line.split(',')
            fields = [field.strip() for field in splits]
            # print(splits, fields)
            ...
            bucket = sects.setdefault(head, [])
            bucket.append(parser(fields))
...
def counting_components(g):
    ...
    >>> counting_components(g)
    1
    >>> g1 = Graph({2, 3, 1, 4, 5, 6}, [(1,2,), (2,1,), (3,4,), (4,3,), (3,5,), (5,3,), (4,5,), (5,4,)])
    ...
    for i, vertex in enumerate(vertices):
        connectors = buckets.get(vertex, None)
        if connectors is None:
            continue
        
        found = search(g, vertex)

        for neigh in found:
            buckets.pop(neigh, -1)
            connectors.add(neigh)

        buckets[vertex] = connectors
...
def deserialize_graph(filename="edmonton-roads-2.0.1.txt"):
    ...
    for v_dict in vx:
        vId = v_dict.get('id', -1)
        vertices.append(vId)
        vertex_map[vId] = v_dict

    ex = sects.get('e', [])
    edges = []

    for e_dict in ex:
        start, end = to_id(e_dict.get('start', -1)), to_id(e_dict.get('end', -1))
        edges.append((start, end,))
        edges.append((end, start,))

    return Graph(vertices, edges), vertex_map

def testing():
    g, vmap = deserialize_graph()
    print(vmap)
    '''
    g = Graph({1, 2, 3, 4, 5, 6, 7}, [(1,2,), (2,1,), (3,4,), (4,3,), (3,5,), (5,3,), (4,5,), (5,4,)])
    print(counting_components(g))
    g.add_edge((1, 4))
    g.add_edge((4, 1))
    '''
    # print(counting_components(g))
